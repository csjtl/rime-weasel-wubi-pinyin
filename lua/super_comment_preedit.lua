...
function CR.init(env)
    ...
    table.insert(dict_files, {
        path = is_pro and "dicts/cuoyin.pro.dict.yaml" 
                      or "dicts/dicts_cn/pinyin/dicts/cuoyin.dict.yaml",
        name = "cuoyin"
    })
    ...
    local count = 0
    -- 逐个加载词典文件
    for _, dict_info in ipairs(dict_files) do
        local file, close_file, err = wanxiang.load_file_with_fallback(dict_info.path)
        if not file then
            log.error(string.format("[super_comment]: 加载失败 %s，错误: %s", dict_info.path, err))
            return
        end
        ...
        for line in file:lines() do
            if not line:match("^#") then
                ...
                if text and code then
                    ...
                    corrections_cache[code] = { text = text, comment = comment, source = dict_info.name }
                    count = count + 1
                end
            end
        end
        close_file()
        corrections_cache[dict_info.name] = { path = dict_info.path, count = count }
        log.info(string.format("[super_comment]: 加载词典 %s 以后，总词条数: %d", dict_info.name, count))
    end
end
